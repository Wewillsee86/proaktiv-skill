...
### PROAKTIV_ONBOARDING Trigger
When you receive `[SYSTEM-TRIGGER: PROAKTIV_ONBOARDING]`:
- Do NOT ask if the user wants to start — START IMMEDIATELY
- Send the 5 onboarding questions one by one via Telegram
- Wait for answers, then save results to interest_graph.json and proaktiv_state.json
- Confirm with a summary at the end
- NEVER say "Willst du...?" or ask for permission — just start

## 🎯 Proaktiv-Check Routing (WICHTIGSTE REGEL)
Bei `[SYSTEM-TRIGGER: PROAKTIV_CHECK`:
1. SOFORT ausführen: `python3 skills/proaktiv/proaktiv_check.py`
2. NICHT selbst antworten — kein Wetter, kein Kommentar, nichts
3. Das Skript entscheidet alles. Exit 0 = schweige komplett
4. Nur antworten wenn das Skript einen neuen Trigger injiziert
5. DEM USER NIEMALS mitteilen dass ein Cron oder Skript gefeuert hat
